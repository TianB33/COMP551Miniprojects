...
from data.utils import orientation
...
def get_data(entry):
    ...
    amino_ids = np.array(amino_ids)
    pos = entry['coords']['CA']
    
    not_nan_mask = ~np.isnan(pos.sum(axis=-1))
    amino_ids = amino_ids[not_nan_mask]
    pos = pos[not_nan_mask]
    
    center = np.sum(a=pos, axis=0, keepdims=True)/pos.shape[0]
    pos = pos - center
    ori = orientation(pos)

    return protein_name, pos, ori, amino_ids.astype(int)

class FoldDataset(Dataset):
    def __init__(self, data_dir='/tmp/protein-data/fold', seed=0, split='training'):
        self.random_state = np.random.RandomState(seed)
        self.split = split

        # npy_dir = os.path.join(data_dir, 'coordinates', split)
        # fasta_file = os.path.join(data_dir, split+'.fasta')
        ...
        fold_classes = {}
        with open(os.path.join(data_dir, 'class_map.txt'), 'r') as f:
            for line in f:
        ...
        with open(os.path.join(data_dir, split+'.txt'), 'r') as f:
            for line in f:
                arr = line.rstrip().split('\t')
                protein_folds[arr[0].upper()] = fold_classes[arr[-1]]
        ...
        for line in lines:
            ...
            protein_name, pos, ori, amino_ids = get_data(entry)
            self.data.append((protein_name, pos, ori, amino_ids.astype(int)))
            self.labels.append(protein_folds[protein_name.upper()])
...
class FuncDataset(Dataset):
    ...
    def __init__(self, data_dir='/tmp/protein-data/func', seed=0, split='training'):

        self.random_state = np.random.RandomState(seed)
        self.split = split

        # # Get the paths.
        # npy_dir = os.path.join(os.path.join(data_dir, 'coordinates'), split)
        # fasta_file = os.path.join(data_dir, 'chain_'+split+'.fasta')
        ...
        protein_functions = {}
        with open(os.path.join(data_dir, 'chain_functions.txt'), 'r') as f:
            for line in f:
                arr = line.rstrip().split(',')
                protein_functions[arr[0].upper()] = int(arr[1])
        ...
        for line in lines:
            entry = json.loads(line)
            protein_name, pos, ori, amino_ids = get_data(entry)
            if len(amino_ids)>1500:
                print(f"skip protein of length {len(amino_ids)}")
                continue
            self.data.append((protein_name, pos, ori, amino_ids.astype(int)))
            name_parts = protein_name.split('.')
            protein_name = f"{name_parts[0]}.{name_parts[1].upper()}"
            self.labels.append(protein_functions[protein_name.upper()])
...
class GODataset(Dataset):
    ...
    def __init__(self, data_dir='/tmp/protein-data/go', level='mf', percent=95, seed=0, split='train'):

        self.random_state = np.random.RandomState(seed)
        self.split = split

        # # Get the paths.
        # npy_dir = os.path.join(data_dir, 'coordinates')
        # fasta_file = os.path.join(data_dir, split+'.fasta')

        # Mask test set.
        test_set = set()
        if split == "test":
            with open(os.path.join(data_dir, "nrPDB-GO_2019.06.18_test.csv"), 'r') as f:
                head = True
                for line in f:
                    if head:
                        head = False
        ...
        with open(os.path.join(data_dir,f'mf_{split}.jsonl')) as f:
            lines = f.readlines()
        ...
        go_annotations = {}
        self.labels = {}
        with open(os.path.join(data_dir, 'nrPDB-GO_2019.06.18_annot.tsv'), 'r') as f:
                    ...
                    if len(arr) > level_idx:
                        ...
                        for go in protein_go_list:
                            if len(go) > 0:
                                ...
                                go_num[go] += 1
                    self.labels[arr[0].upper()] = np.array(protein_labels)
    ...
    def __getitem__(self, idx):
        ...
        if len(self.labels[protein_name.upper()]) > 0:
            label[self.labels[protein_name.upper()]] = 1.0
...
class ECDataset(Dataset):
    ...
    def __init__(self, data_dir='/tmp/protein-data/ec', percent=95, seed=0, split='train'):
        ...
        test_set = set()
        if split == "test":
            with open(os.path.join(data_dir, "nrPDB-EC_test.csv"), 'r') as f:
                head = True
                for line in f:
                    if head:
                        head = False
        ...
        with open(os.path.join(data_dir,f'ec_{split}.jsonl')) as f:
            lines = f.readlines()
        ...
        with open(os.path.join(data_dir, 'nrPDB-EC_annot.tsv'), 'r') as f:
            for idx, line in enumerate(f):
                if idx == 1:
                    arr = line.rstrip().split('\t')
                    for ec in arr:
